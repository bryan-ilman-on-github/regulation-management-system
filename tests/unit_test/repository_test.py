...
def test_user_repository_get_by_email(db_session: Session):
    ...
    nonexistent = user_repository.get_by_email(
...
def test_regulation_repository_create(db_session: Session):
    ...
    regulation_in = RegulationCreate(
        nama_peraturan="Repo Regulation", judul="Repo Test Judul", tahun="2020"
    )
...
def test_regulation_repository_get(db_session: Session):
    ...
    regulation_in = RegulationCreate(
        nama_peraturan="Get Repo Regulation", judul="Get Repo Test Judul", tahun="2019"
    )
    created = regulation_repository.create(db_session, obj_in=regulation_in)

    # Get the regulation
    regulation = regulation_repository.get(
        db_session, regulation_id=created.regulation_id
    )
