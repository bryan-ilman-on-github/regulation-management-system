...
def test_scrape_regulation_detail(mock_get, mock_response, scraper):
    ...
    assert result['judul'] == "Peraturan Pemerintah (PP) Nomor 12 Tahun 2020 tentang Testing dan Automation"
    ...
    assert result['diubah_dengan'][0]['text'] == "PP No. 15 Tahun 2021 tentang Update Testing"
    assert result['diubah_dengan'][0]['link'] == "https://peraturan.bpk.go.id/Details/12345"
    
    assert len(result['mengubah']) == 1
    assert result['mengubah'][0]['text'] == "PP No. 10 Tahun 2019 tentang Original Testing"
