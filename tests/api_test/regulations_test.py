...
def test_create_regulation(client: TestClient, db_session: Session, auth_headers: dict):
    regulation_data = {
        "nama_peraturan": "Test Peraturan",
        "judul": "Judul Test",
        "tahun": "2025",
    }
    response = client.post(
        "/api/v1/regulations/", json=regulation_data, headers=auth_headers
    )
    assert response.status_code == 201
    data = response.json()
...
def test_read_single_regulation(client: TestClient, auth_headers: dict):
    ...
    regulation_data = {
        "nama_peraturan": "Peraturan Test",
        "judul": "Judul Test Read Single",
        "tahun": "2023",
    }
...
def test_update_regulation(client: TestClient, auth_headers: dict):
    ...
    # First create a regulation
    regulation_data = {
        "nama_peraturan": "Peraturan Update",
        "judul": "Judul Before Update",
        "tahun": "2022",
    }
    create_response = client.post(
        "/api/v1/regulations/", json=regulation_data, headers=auth_headers
    ...
    response = client.put(
        f"/api/v1/regulations/{regulation_id}", json=update_data, headers=auth_headers
    )
    assert response.status_code == 200
...
def test_update_regulation_unauthenticated(client: TestClient):
    ...
    random_id = str(uuid.uuid4())
    response = client.put(
        f"/api/v1/regulations/{random_id}", json={"judul": "Unauthorized Update"}
    )
...
def test_delete_regulation(client: TestClient, auth_headers: dict):
    ...
    # First create a regulation
    regulation_data = {
        "nama_peraturan": "Peraturan Delete",
        "judul": "Judul For Delete",
        "tahun": "2021",
    }
    create_response = client.post(
        "/api/v1/regulations/", json=regulation_data, headers=auth_headers
    ...
    response = client.delete(
        f"/api/v1/regulations/{regulation_id}", headers=auth_headers
    )
    assert response.status_code == 200
