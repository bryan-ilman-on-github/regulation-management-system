...
def get_last_page_number(session: requests.Session, url: str) -> int | None:
    ...
    try:
        ...
        last_page_link = soup.find('a', class_='page-link', string=re.compile(r'\s*Last\s*'))
        
        if not last_page_link:
            logging.warning("Could not find the 'Last' page link. Scraping may be incomplete.")
            return None
...
def get_all_regulation_links() -> list[str]:
    ...
    logging.info(f"Finished scraping all pages. Found a total of {len(all_links)} unique regulation links.")
...
def populate_database():
    ...
    for i, link in enumerate(regulation_links):
        ...
        existing = db.query(Regulation).filter(Regulation.link_peraturan == link).first()
        if existing:
