...
def create_regulation(regulation_in: RegulationCreate, db: Session = Depends(get_db)):
...
def update_regulation(regulation_id: UUID, regulation_in: RegulationUpdate, db: Session = Depends(get_db)):
    return service.update_regulation(db=db, regulation_id=regulation_id, regulation_in=regulation_in)

@router.delete("/{regulation_id}", response_model=Regulation)
def delete_regulation(regulation_id: UUID, db: Session = Depends(get_db)):
